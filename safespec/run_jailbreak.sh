...
#   RUN_MODE=speculative bash run_jailbreak.sh                  # 换模式: spec_safe(默认,带安全头) / speculative(原型) / target_only / draft_only
...
BASE_MODEL_NAME="${BASE_MODEL_NAME:-Qwen/Qwen3-32B}"
SMALL_MODEL_NAME="${SMALL_MODEL_NAME:-Qwen/Qwen3-1.7B}"
...
SAFETY_MODEL_PATH="${SAFETY_MODEL_PATH:-Qwen/Qwen3-32B}"
SAFETY_HEAD_PATH="${SAFETY_HEAD_PATH:-${DATA_DIR}/safety_head_ckpts/safety_head_ckpt_qwen32b_layerlast_mix_aug2/safety_head_best.pt}"
...
# 运行参数
RUN_MODE="${RUN_MODE:-spec_safe}"  # 可选: speculative, spec_safe, draft_only, target_only, transformers_only, psr, safedecoding, secdecoding
...
if [ "$RUN_MODE" == "secdecoding" ]; then
    SEC_ARGS="--sec_large_model_path $BASE_MODEL_NAME --sec_small_base_path $SMALL_MODEL_NAME --sec_small_expert_path $SMALL_MODEL_NAME --sec_small_expert_lora /path/to/SafeDecoding/lora_modules/deepseek_8b --sec_alpha 1.2 --sec_first_n_tokens 100 --sec_signal_stride 1 --sec_unsafe_threshold 0.15"
fi
...
if [ "$RUN_MODE" == "spec_safe" ] && [ ! -f "$SAFETY_HEAD_PATH" ]; then
    echo "spec_safe 需要 safety head，但当前不存在：$SAFETY_HEAD_PATH"
